...
class FileAgent:
    ...
    def handle(self, step: ToolCall | dict) -> ToolResult:
        ...
        self.logger.debug(f"[FileAgent] {call.tool}({call.args})")
        try:
            ...
            self.logger.error(f"[FileAgent error] {call.tool}: {e}")
